...
TEST_IMAGE_URL = 's3://mlops-aws/input-images/4.jpg'
...
def get_baseline_throughput(batch_size, num_iterations=10):
    ...
    model = TimmClassifier.load_from_checkpoint(model_file)
...
def prepare_test_payload():
    """Prepare a test image payload from S3"""
    s3_bucket = 'mlops-aws'  # Your S3 bucket name
    s3_key = 'input-images/4.jpg'  # Your S3 object key

    # Create an S3 client
    s3 = boto3.client('s3')

    # Fetch the image from S3
    img_data = s3.get_object(Bucket=s3_bucket, Key=s3_key)['Body'].read()
